...
export default function RealtimeScannerClient() {
  ...
  const handleScan = useCallback(async (uidToScan: string) => {
    if (!uidToScan) {
      toast({
        variant: 'destructive',
        title: 'Erro',
        description: 'UID do cartão está vazio.',
      });
      return;
    }
    setIsLoading(true);
    ...
    try {
      const authResult = await checkAccess({ cardUID: uidToScan });
  ...
  }, [lastMessage]); // handleScan is stable due to useCallback
